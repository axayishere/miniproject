...
install.packages(c("shiny", "flexdashboard", "tidyquant", "tidyverse", "plotly", "scales", "DT", "quantmod", "zoo"))
...
```{r data-collection}
# 3. Data Collection Logic (Offline-First)
...
for (ticker in tickers) {
  file_path <- file.path(data_dir, paste0(ticker, ".csv"))
  
  if (file.exists(file_path)) {
    # LOAD FROM LOCAL CSV
    df <- read_csv(file_path, show_col_types = FALSE)
    # Standardize column names to Title Case
    df <- df %>% rename_with(str_to_title) %>% mutate(Ticker = ticker)
    stock_data_list[[ticker]] <- df
  } else {
    # DOWNLOAD IF MISSING
    tryCatch({
      df_raw <- tq_get(ticker, from = Sys.Date() - 180, to = Sys.Date())
      if (nrow(df_raw) > 0) {
        write_csv(df_raw, file_path)
        df_formatted <- df_raw %>% rename_with(str_to_title) %>% mutate(Ticker = ticker)
        stock_data_list[[ticker]] <- df_formatted
      }
    }, error = function(e) { message(paste("Error downloading", ticker)) })
  }
}
